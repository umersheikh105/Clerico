...
public class AddInvoiceController implements Initializable{
	...
	public void addInvoice(ActionEvent event) throws SQLException {
		...
		if(laborCost.isEmpty()||signature.isEmpty()||partChoiceBox.getValue().equals(null)||clientChoiceBox.getValue().equals(null)||notes.isEmpty()) {
		...
		ResultSet rs = connection.executeQuery("select part_id FROM Part WHERE part_name = " +  "'" + partChoiceBox.getValue() + "'");
		int partID = 0;
		try {
			rs.next();
		...
		String sqlInsert = "INSERT INTO Form (part_id, labor_cost, date, notes) VALUES (" 
				+ "'"+ partID + "'," 
				+ "'"+ Integer.parseInt(laborCost) + "'," 
				+ "'"+ today + "'," 
				+ "'"+ notes + "'"
				+ ")";
		...
		rs = connection.executeQuery("select client_id from Client where firstname = " +  "'" + clientChoiceBox.getValue()+ "'");
		rs.next();
		int clientID = rs.getInt("client_id");
		
		
		String clientFormInsert = "INSERT INTO ClientForm (form_id, client_id, signature) VALUES (" 
				+ "'"+ formID + "'," 
				+ "'"+ clientID + "'," 
				+ "'"+ signature + "'" 
				+ ")";
		...
		if(connection.executeAction(clientFormInsert)) {
			...
			alert.setHeaderText(null);
			alert.setContentText("Success " + " Total Cost: " + String.valueOf(Integer.parseInt(textFieldLaborCost.getText()) + rs.getInt("cost")));
			alert.showAndWait();	
		}
		else {
			Alert alert = new Alert(Alert.AlertType.ERROR);
